...
void main() {
  group('UpdateService.checkForUpdate', () {
    ...
    test('includes macOS zip asset metadata when present', () async {
      final service = UpdateService(
        packageInfoLoader: () async => PackageInfo(
          appName: 'Dacx',
          packageName: 'run.rosie.dacx',
        ...
        httpGet: (uri, {headers}) async => http.Response('''
          {
            ...
            "assets": [
              {
                "name": "Dacx-macOS.zip",
                "browser_download_url": "https://github.com/BurntToasters/Dacx/releases/download/v0.6.0/Dacx-macOS.zip",
                "size": 456,
      ...
      expect(update, isNotNull);
      expect(update!.hasMacOsZip, isTrue);
      expect(update.macOsZipAssetName, 'Dacx-macOS.zip');
      expect(update.macOsZipSize, 456);
      expect(update.macOsZipSha256, 'def456');
      expect(update.macOsZipUrl, endsWith('Dacx-macOS.zip'));
    });
