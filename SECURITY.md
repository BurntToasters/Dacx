...
- **Self-update** — downloads from GitHub (host allowlist). **Windows:** SHA256 must match both `SHA256SUMS` and an **Ed25519-signed update manifest** (this is the primary install trust). **Authenticode** is an optional extra pin when `WINDOWS_SIGNING_CERT_THUMBPRINT` is baked into the binary; default GitHub MSI builds may ship without Authenticode (SmartScreen may warn). **macOS:** SHA256SUMS over TLS, then `codesign --verify --deep --strict` plus Team ID / bundle ID / version / Gatekeeper checks via the XPC helper. **Linux:** no in-app self-update; trust GPG-signed packages / Flatpak / manual downloads.
